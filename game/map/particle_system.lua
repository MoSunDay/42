...
function ParticleSystem.new()
    ...
    self.waterSparkles = {}
    self.season = "spring"
    self.config = SEASON_CONFIGS.spring
    
    self.wind = 0
    self.windTarget = 0
    self.windChangeTimer = 0
...
function ParticleSystem:setSeason(season)
    self.season = season
    self.config = SEASON_CONFIGS[season] or SEASON_CONFIGS.spring
    self:clearParticles()
end

function ParticleSystem:clearParticles()
    self.particles = {}
    self.waterSparkles = {}
end
...
function ParticleSystem:createParticle(x, y, customConfig)
    local config = customConfig or self.config
    
    local particle = {
        x = x,
        y = y,
        vx = self:randomRange(config.speed.x.min, config.speed.x.max),
        vy = self:randomRange(config.speed.y.min, config.speed.y.max),
        size = self:randomRange(config.size.min, config.size.max),
        color = config.colors[math.random(1, #config.colors)],
        lifetime = self:randomRange(config.lifetime.min, config.lifetime.max),
        age = 0,
        rotation = config.rotation and math.random() * math.pi * 2 or 0,
        rotationSpeed = config.rotation and self:randomRange(config.rotationSpeed.min, config.rotationSpeed.max) or 0,
    ...
    if config.wobble then
        ...
        particle.wobbleSpeed = self:randomRange(3, 6)
    end
...
function ParticleSystem:randomRange(min, max)
    return min + math.random() * (max - min)
end

function ParticleSystem:emit(x, y, count, customConfig)
    ...
    for i = 1, count do
        if #self.particles < config.maxParticles then
            local offsetX = (customConfig and 0) or math.random(-50, 50)
            local offsetY = (customConfig and 0) or math.random(-20, 20)
            table.insert(self.particles, self:createParticle(x + offsetX, y + offsetY, config))
        end
    end
end

function ParticleSystem:emitWaterSparkle(x, y, width, height)
    if not self.enabled then return end
    if #self.waterSparkles >= WATER_SPARKLE_CONFIG.maxSparkles then return end
    
    local sparkle = {
        x = x + math.random() * width,
        y = y + math.random() * height,
        size = self:randomRange(WATER_SPARKLE_CONFIG.size.min, WATER_SPARKLE_CONFIG.size.max),
        color = WATER_SPARKLE_CONFIG.colors[math.random(1, #WATER_SPARKLE_CONFIG.colors)],
        lifetime = self:randomRange(WATER_SPARKLE_CONFIG.lifetime.min, WATER_SPARKLE_CONFIG.lifetime.max),
        age = 0,
        phase = math.random() * math.pi * 2
    }
    
    table.insert(self.waterSparkles, sparkle)
end

function ParticleSystem:updateWind(dt)
    self.windChangeTimer = self.windChangeTimer + dt
    
    if self.windChangeTimer > 3 then
        self.windChangeTimer = 0
        self.windTarget = self:randomRange(-20, 20)
    end
    
    self.wind = self.wind + (self.windTarget - self.wind) * dt * 0.5
end
...
function ParticleSystem:update(dt)
    ...
    self:updateParticles(dt, self.particles, self.config)
    self:updateParticles(dt, self.waterSparkles, WATER_SPARKLE_CONFIG)
end

function ParticleSystem:updateParticles(dt, particleList, config)
    local i = 1
    while i <= #particleList do
        ...
        if p.age >= p.lifetime then
            ...
            local swayX = 0
            if config.sway then
                swayX = math.sin(self.time * config.sway.frequency + p.swayOffset) * config.sway.amplitude * dt
            end
            
            p.x = p.x + (p.vx + self.wind) * dt + swayX
            p.y = p.y + p.vy * dt
...
function ParticleSystem:emitFromTop(screenWidth, cameraX, cameraY, mapWidth, mapHeight)
    if not self.enabled then return end
    
    local emitCount = math.floor(self.config.emitRate * 0.1)
    if math.random() < (self.config.emitRate * 0.1 - emitCount) then
        emitCount = emitCount + 1
    end
    
    for i = 1, emitCount do
        if #self.particles < self.config.maxParticles then
            local x = cameraX + math.random() * screenWidth + self.wind * 2
            local y = cameraY - 20
...
function ParticleSystem:draw(camera)
    ...
    for _, s in ipairs(self.waterSparkles) do
...
function ParticleSystem:drawParticle(p)
    ...
    if p.rotation and p.rotation ~= 0 then
        ...
        if self.config.type == "petals" then
            love.graphics.ellipse("fill", 0, 0, p.size, p.size * 0.6)
        elseif self.config.type == "leaves" then
...
function ParticleSystem:drawSparkle(s)
...
function ParticleSystem:getParticleCount()
    return #self.particles, #self.waterSparkles
end

function ParticleSystem:setWind(wind)
    self.windTarget = wind
end
