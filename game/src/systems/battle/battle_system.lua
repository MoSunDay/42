...
local DOT_PERCENT_TO_FLAT_MIN = 50
...
function BattleSystem.process_status_effects(state)
    ...
    if player.skillDot then
        ...
        local maxHp = player.maxHp or 100
        local dotDamage = math.max(1, math.floor((dot.damage or 0) * maxHp))
        ...
        if player.hp <= 0 then
            BattleSystem.add_log(state, "You collapsed from " .. (dot.type or "dot") .. "!")
            BattleSystem.end_battle(state, BATTLE_STATE.DEFEAT)
            return
        end
    end
    ...
    for _, enemy in ipairs(state.enemies) do
        if Enemy.is_alive(enemy) then
            if enemy.skillDot then
                ...
                if enemy.hp <= 0 then
                    BattleSystem.add_log(state, enemy.name .. " defeated by " .. (dot.type or "dot") .. "!")
                end
            end
        if not Player.is_alive(player) then
            BattleSystem.end_battle(state, BATTLE_STATE.DEFEAT)
            return "defeat"
        end
    end
    ...
    for _, enemy in ipairs(state.enemies) do
        if Enemy.is_alive(enemy) then
            if enemy.skillDot then
                local dot = enemy.skillDot
                local dotDamage = math.max(1, math.floor((enemy.maxHp or 100) * (dot.damage or 0)))
                enemy.hp = math.max(0, enemy.hp - dotDamage)
                BattleSystem.add_log(state, string.format("%s takes %d damage from %s!", enemy.name, dotDamage, dot.type or "dot"))
                dot.duration = dot.duration - 1
                if dot.duration <= 0 then
                ...
                if not Enemy.is_alive(enemy) then
                    BattleSystem.add_log(state, enemy.name .. " defeated!")
                end
            end
        if player.hp <= 0 then
            BattleSystem.end_battle(state, BATTLE_STATE.DEFEAT)
    ...
    for _, enemy in ipairs(state.enemies) do
        if Enemy.is_alive(enemy) then
            if enemy.skillDot then
                local dot = enemy.skillDot
                local rawDamage = dot.damage or 0
                local dotDamage = rawDamage < 1
                    and math.max(1, math.floor(enemy.maxHp * rawDamage))
                    or math.floor(rawDamage)
                enemy.hp = math.max(0, enemy.hp - dotDamage)
                BattleSystem.add_log(state, string.format("%s takes %d damage from %s!", enemy.name, dotDamage, dot.type or "dot"))
                dot.duration = dot.duration - 1
                if dot.duration <= 0 then
    ...
    for _, enemy in ipairs(state.enemies) do
        if Enemy.is_alive(enemy) then
            if enemy.skillDot then
                local dot = enemy.skillDot
                local dotDamage = math.max(1, math.floor((dot.damage or 0) * enemy.maxHp))
                enemy.hp = math.max(0, enemy.hp - dotDamage)
                BattleSystem.add_log(state, string.format("%s takes %d damage from %s!", enemy.name, dotDamage, dot.type or "dot"))
                dot.duration = dot.duration - 1
                if dot.duration <= 0 then
        ...
        if not Player.is_alive(player) then
            BattleSystem.add_log(state, "You succumb to the damage over time!")
            BattleSystem.end_battle(state, BATTLE_STATE.DEFEAT)
            return true
        end
    end
    ...
    for _, enemy in ipairs(state.enemies) do
        if Enemy.is_alive(enemy) then
            if enemy.skillDot then
                local dot = enemy.skillDot
                local dotDamage = math.ceil(dot.damage or 0)
                if dotDamage > 0 then
                    enemy.hp = math.max(0, enemy.hp - dotDamage)
                    BattleSystem.add_log(state, string.format("%s takes %d damage from %s!", enemy.name, dotDamage, dot.type or "dot"))
                end
        ...
        if not Player.is_alive(player) then
            BattleSystem.add_log(state, "You succumb to " .. (dot.type or "dot") .. "!")
        end
    end
    ...
    for _, enemy in ipairs(state.enemies) do
        if Enemy.is_alive(enemy) then
            if enemy.skillDot then
                ...
                local dotDamage
                if rawDamage < 1 then
                    dotDamage = math.max(1, math.floor(enemy.maxHp * rawDamage))
                else
                    dotDamage = math.floor(rawDamage)
                end
                if dotDamage > 0 then
                    enemy.hp = math.max(0, enemy.hp - dotDamage)
                    BattleSystem.add_log(state, string.format("%s takes %d damage from %s!", enemy.name, dotDamage, dot.type or "dot"))
